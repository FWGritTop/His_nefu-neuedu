    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
            ...
            size="mini"
            @click="handleAdd"
            v-hasPermi="['system:drugslist:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
            ...
            :disabled="single"
            @click="handleUpdate"
            v-hasPermi="['system:drugslist:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
            ...
            :disabled="multiple"
            @click="handleDelete"
            v-hasPermi="['system:drugslist:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
            ...
            size="mini"
            @click="handleExport"
            v-hasPermi="['system:drugslist:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="drugslistList" @selection-change="handleSelectionChange">
...
<script>
  import { listDrugslist, getDrugslist, delDrugslist, addDrugslist, updateDrugslist } from "@/api/system/drugslist";
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        listDrugslist(this.queryParams).then(response => {
          this.drugslistList = response.records;
        ...
        this.$refs["form"].validate(valid => {
          if (valid) {
            if (this.form.id != null) {
              updateDrugslist(this.form).then(response => {
                this.$modal.msgSuccess("修改成功");
                this.open = false;
                this.getList();
              });
            } else {
              addDrugslist(this.form).then(response => {
      ...
      handleDelete(row) {
        ...
        this.$modal.confirm('是否确认删除成品药编号为"' + ids + '"的数据项？').then(function() {
          return delDrugslist(ids);
        }).then(() => {
          this.getList();
  ...
  /** 导出按钮操作 */
  handleExport() {
    this.download('system/drugslist/export', {
      ...this.queryParams
    }, `drugslist_${new Date().getTime()}.xlsx`)
  }
  }
  };
</script>
